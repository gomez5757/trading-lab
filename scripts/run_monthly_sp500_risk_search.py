...
from trading_lab.monthly_risk import (  # noqa: E402
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run one monthly SP500 risk search stage.")
    parser.add_argument("--config", default="configs/monthly_sp500_risk.yaml")
    ...
    output_dir = Path("outputs/monthly_sp500_risk")
    rows = []
    examples_count = 0
    try:
        raw_config = load_yaml(args.config)
        data_path = raw_config.get("data_path", "data/public/sp500_annual_daily.csv")
        output_dir = Path(args.output_dir or raw_config.get("output_dir", "outputs/monthly_sp500_risk"))
        daily = load_market_data(data_path)
        examples = build_monthly_examples(
            daily,
        ...
        config = MonthlyRiskSearchConfig(
            ...
            random_seed=int(args.random_seed or raw_config.get("random_seed", 412_000)),
            top_rows_per_stage=int(args.top_rows_per_stage or raw_config.get("top_rows_per_stage", 250)),
        )
        rows = run_monthly_risk_search(examples, config)
        write_monthly_risk_outputs(rows, examples, output_dir, stage=args.stage)
    except Exception as exc:
        output_dir.mkdir(parents=True, exist_ok=True)
        rows = [
            {
                "candidate_id": f"monthly_risk_stage_error_{args.stage}",
                "stage": args.stage,
                "stage_failed": True,
                "stage_error": str(exc),
                "monthly_risk_score": -1_000_000.0,
                "accepted": False,
                "locked_opened": False,
            }
        ]
        (output_dir / "monthly_risk_summary.json").write_text(
            json.dumps(
                {
                    "stage": args.stage,
                    "stage_failed": True,
